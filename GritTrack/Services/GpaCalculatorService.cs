...
namespace GritTrack.Services
{
    ...
    public class GpaCalculatorService
    {
        ...
        {
            // Bolt optimization: Replaced chained LINQ operations (.Where().ToList() + multiple .Sum()s)
            // with a single foreach loop. This eliminates intermediate list allocations and reduces
            // passes from O(3N) to O(N).
            double earned = 0;
            double possible = 0;
            int count = 0;
